...
async function performVectorSearch(queryEmbedding, limit = 10, filters = {}, collectionName = "embeddings") {
  ...
  while (hasMoreDocuments) {
    ...
    // Compute queryEmbedding's dimensionality once, before the
    // forEach loop, so each document can be filtered for
    // dimension-mismatch before cosineSimilarity is called. The
    // cosineSimilarity helper loops by vecA.length and reads vecB[i]
    // without bounds-checking, so a document whose embedding has a
    // different length than the query (which can happen if the
    // embeddings collection contains documents generated by a
    // different model -- text-embedding-3-small is 1536-dim,
    // text-embedding-3-large is 3072-dim, ada-002 is 1536-dim,
    // gte-small is 384-dim) would read past the end of the shorter
    // array, produce a NaN similarity, and then NaN-poison the result
    // sort because NaN comparisons in JavaScript Array.sort are
    // unstable. Same fix pattern landed in the MCP server's
    // searchTranscripts in commit 4bc8680.
    ...
    batchSnapshot.forEach((doc) => {
      ...
      if (data.embedding && Array.isArray(data.embedding) && data.embedding.length === queryDim) {
        ...
        if (passesFilters) {
          ...
          // Defensive: skip non-finite similarity values (NaN, Infinity)
          // that could result from edge-case inputs. The dimension
          // filter above should prevent NaN in normal cases, but
          // belt-and-suspenders here protects against any future
          // change to cosineSimilarity that introduces a new NaN path.
          if (!Number.isFinite(similarity)) {
            return; // continue to next doc in forEach
          }

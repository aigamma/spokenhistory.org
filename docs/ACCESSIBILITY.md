...
The `prefers-reduced-motion` rule above only follows the OS setting and, by construction, cannot pause a `<video>`: it collapses CSS animation and transition durations, but video playback is not a CSS animation. The landing-page timeline auto-plays continuous, looping motion (about eleven period "GIF" videos plus a few infinite CSS animations) that runs well past five seconds, so SC 2.2.2 wants an in-page mechanism to pause it. A header toggle, shown on the timeline page where that motion lives (2026-06-03), now provides one, and the same preference keeps the content-dense footer calm.

- **Preference plumbing** (`src/hooks/useAnimationPreference.js`): mirrors the light/dark theme idiom exactly. A writer hook (`useAnimationPreference`) toggles and persists the choice to `localStorage` (`animationsPaused`); a read-only hook (`useAnimationsPaused`) subscribes via `MutationObserver`. The choice is reflected onto `data-animations-paused` on `<html>`. With no stored choice the default follows the OS `prefers-reduced-motion` setting, so a motion-sensitive visitor lands on an already-calm page without having to find the control first. In practice this means animations default ON for everyone except visitors whose OS asks for reduced motion: that is intentional (Eric, 2026-06-03), because a default-on state makes the control read "Pause Animations" (a clear, discoverable affordance) rather than leading with the resume-state "Allow Animations" label. Devices with a detectable reduce-motion setting are the deliberate exception and still default to paused.
...
- **The control** (`src/components/common/Header.jsx`): a clear "Pause Animations" / "Allow Animations" pill in the top-right chrome cluster, just left of the Dark toggle, rendered ONLY on the timeline page (`/`) where the looping motion lives (a `location.pathname === '/'` guard; on every other page the button is absent because there is nothing there to pause). Icon plus label name the action the button performs; `aria-pressed` reflects state. The label is visible at **every** width, including the narrowest phones: a motion-sensitive visitor has to be able to see that a pause exists, or they bounce (or reach for an ad blocker) before they would ever find a bare icon. Room is made by dropping the Dark toggle to its icon below `sm` and letting the chrome row wrap (the Dark+Menu group falls to a tidy, right-aligned second line) rather than clipping the label.

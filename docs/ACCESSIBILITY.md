...
- **The control** (`src/components/common/Header.jsx`): a clear "Pause Animations" / "Play Animations" pill in the top-right chrome cluster, just left of the Dark toggle, rendered ONLY on the timeline page (`/`) where the looping motion lives (a `location.pathname === '/'` guard; on every other page the button is absent because there is nothing there to pause). Icon plus label name the action the button performs; `aria-pressed` reflects state. The label is visible at **every** width, including the narrowest phones: a motion-sensitive visitor has to be able to see that a pause exists, or they bounce (or reach for an ad blocker) before they would ever find a bare icon. Room is made by dropping the Dark toggle to its icon below `sm` and letting the chrome row wrap (the Dark+Menu group falls to a tidy, right-aligned second line) rather than clipping the label.

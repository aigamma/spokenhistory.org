...
def end_timestamp_for_quote(quote, cues, big, spans):
    """Locate the quote in the concatenated cue text; return the formatted
    end timestamp (last spanned cue end + tail) or None if not found."""
    nq = norm(quote)
    if not nq:
        return None
    pos = big.find(nq)
    ...
    if cue_idx is None:
        return None
    return fmt_hms(cues[cue_idx]["end"] + TAIL_SECONDS)
    ...
    for f in sorted(PEOPLE.glob("*.json")):
        ...
        for sn in snips:
            ...
            if srt is not None and quote:
                ...
                end_ts = end_timestamp_for_quote(quote, cues, big, spans)
            if end_ts:
                aligned += 1
            else:
                unaligned += 1

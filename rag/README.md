...
`entry_provenance` lets retrieval differentiate the 127 entries that went through the full Pass 1–8 audit cascade (`audit-original`) from the entries that came in via the streamlined ingestion (measured value: `ingestion-loc-verified` on the nine 2026-05-25 onboards; the four 2026-05-31 onboards carry a placeholder pending backfill). For Smithsonian-grade publication, an LLM answer that draws from an audit-original chunk can cite the audit overlay; an answer drawn from an ingestion-only chunk should be hedged. Putting the flag in the chunk metadata keeps it available at answer time without a second lookup.

`inferential_uncertainty_score` + `inferential_uncertainty_tier` carry the per-entry residual-error estimate defined in `transcripts/AUDIT_TRAIL.md::Inferential scoring framework`. The measured tier distribution (2026-06-10, pipeline_output) is `high` 133, `not-auditable` 3, `ingestion-only` 3, `publication-block` 1 over 140 entries; `high` became the dominant value after the Pass 10 rescore credited the LoC line-by-line cross-reference (`low`/`medium` were earlier-era values). Per-tier counts shift as new interviews are ingested; verify against the current manifests. Retrieval can use the tier as a coarse filter (e.g., `tier: {$eq: "high"}` to bias toward publication-grade entries) or weight rerank by score. Note: the user-facing `/rag-explore` UI collapses these into two settled states, "LoC-Verified" (137) and "Audio-Limited" (3), for a 140-interview total.
...
- ✅ **`/retrieve` Netlify Function deployed.** Citation-grade payloads with `entry_number` shortcut + `dedupeByEntry` polyphonic option. Live at `https://civil-rights-staging.netlify.app/retrieve`; production frontend at spokenhistory.org.

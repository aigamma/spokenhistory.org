...
**Current catalog: 202 pages (165 interviewees + 37 external figures, as of 2026-06-10).** Every named CRHP interviewee plus every named external figure referenced in FamousNames, the influence graph, OR repeatedly mentioned across multiple interviewee bios has a catalog entry. Hero imagery is complete (0 bare pages, per the 2026-06-01 status). **Bio coverage: 202 of 202 pages (100%) have a citation-bearing biographical paragraph** grounded in external scholarly sources (university-press monographs, Mississippi/Oklahoma/Tennessee state encyclopedias, SNCC Digital Gateway, Veterans of the Civil Rights Movement archive, Civil Rights Digital Library, NMAAHC object records, NYT obituaries, etc.); no Wikipedia citations are used per the writing discipline below. The audit-tier protection language is retained on publication-block / not-auditable / ingestion-only tier transcripts to caution future researchers that any specific quoted claim drawn from the LoC transcript itself should be verified against external sources before publication.
  ...
  "interview_snippets": [
    {
      ...
      "end_timestamp": "01:51:27",        // clip end; added by scripts/add_snippet_clip_bounds.py
...
- Photographers whose work is **still copyrighted** and must NOT be hosted: Charles Moore, Spider Martin, Bob Adelman, Joseph Louw, Magnum agency photographers (Bruce Davidson, Danny Lyon's earlier press work), most AP/UPI/Getty CRM-era press archive material. When no usable PD photo exists, leave the `photo` field absent. **The page does NOT fabricate a placeholder portrait.** PersonPage promotes the first `gallery[]` image (environment or association imagery) into the hero slot instead, and if there is no gallery either, the page opens on the identity header with no hero image. It never renders a giant initial-letter avatar or a no-photo apology caption (that earlier behavior was removed 2026-05-29 after it shipped an absurd single-letter box plus an explanatory caption at the very top of every portrait-less page). **For any portrait-less page, source at least one strong environment or association image into `gallery[]`** (a place, an institution, a courthouse, a march or a meeting site, an associated figure who has a PD/open-licensed photo) so the page still opens on contextual imagery rather than a bare header.
...
The PersonPage component renders the `photo` block as a centered hero figure below the identity header, full prose-column width, with the full attribution (photographer, repository, license) in a figcaption beneath the image; CC-BY and CC-BY-SA images surface the photographer credit automatically, which satisfies the attribution requirement. When `photo` is absent, the first `gallery[]` image takes the hero slot (and is then not repeated lower in the body); when neither `photo` nor `gallery[]` exists, no hero figure renders and the identity header leads.

**The Wikimedia URL path prefix is not free text, copy it, never hand-write it.** A Commons original is served at `https://upload.wikimedia.org/wikipedia/commons/<a>/<ab>/<Filename>`, where `<a>/<ab>` is the first hex digit and the first two hex digits of the MD5 of the (underscore-form) filename. A wrong prefix is a guaranteed 404 even when the file exists, and it fails silently: the catalog never fetches the image at build time, so a broken URL looks fine in the JSON and only shows up as a broken image in the browser. An early enrichment pass fabricated these prefixes and silently broke 51 images across 27 pages (fixed 2026-06-01). Always take the full `upload.wikimedia.org/...` URL verbatim from the Commons file page (the WebFetch in step 2 above returns it); do not reconstruct it from the filename by guessing the prefix. **Run `node scripts/verify_image_urls.mjs` after adding or editing any page with Wikimedia imagery**, it is an offline, deterministic guard that recomputes the canonical prefix for every Commons URL in the catalog and fails on any disagreement (no network, safe for CI). It checks prefix correctness, not Commons existence; if the recomputed-prefix URL still 404s in a browser, the filename itself is wrong and the image needs re-sourcing.

  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-64">
        ...
        <p className="text-gray-600 dark:text-zinc-400">Loading first video...</p>
      </div>
    );
  }
          ...
          <div className="w-full lg:flex-shrink-0 lg:max-w-[960px]">
            <div className="w-full aspect-video relative rounded-xl overflow-hidden mb-4">
              {currentVideo ? (
                ...
                <div className="w-full h-full bg-gray-400 dark:bg-zinc-800 flex items-center justify-center text-white text-xl">
                  No video available
                </div>
              )}
            </div>
                 ...
                 {getCurrentVideoTags().length > 0 ? (
                   getCurrentVideoTags().map((tag, index) => (
                     <button
                       type="button"
                       key={index}
                       className="px-4 sm:px-6 py-3 min-h-11 rounded-[50px] outline outline-1 outline-offset-[-1px] outline-black dark:outline-zinc-600 inline-flex justify-center items-center gap-2.5 cursor-pointer hover:bg-civil-red-strong hover:outline-civil-red-strong transition-colors duration-200 group bg-transparent"
                       onClick={() => navigate(`?keywords=${encodeURIComponent(tag)}`)}
                       aria-label={`Filter playlist to topic ${tag}`}
                     >
                       <span className="text-center text-black dark:text-zinc-100 text-sm sm:text-base font-light font-['Chivo_Mono'] group-hover:text-white capitalize">
                         {tag}
                       </span>
                     </button>
                   ))
                 ) : (
                   <span className="text-gray-500 dark:text-zinc-400 text-base font-light font-mono italic">
            ...
            {currentVideo && (
              <>
                ...
                <div className="mb-6 flex-shrink-0">
                  <h2 className="text-black dark:text-zinc-100 text-3xl sm:text-4xl md:text-5xl font-semibold capitalize" style={{fontFamily: 'Inter, sans-serif'}}>
                    {currentVideo.name}
                  </h2>
                </div>
                ...
                <div className="text-black dark:text-zinc-200 text-lg sm:text-xl md:text-2xl font-normal leading-relaxed flex-1 pr-2 sm:pr-4 lg:max-h-[480px] lg:overflow-y-auto" style={{fontFamily: 'FreightText Pro, serif'}}>
        ...
        <div className="w-full">
          <div className="mb-14">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-black dark:text-zinc-100 text-3xl sm:text-4xl md:text-5xl font-medium" style={{fontFamily: 'Inter, sans-serif'}}>Playlist</h2>
            ...
            <div className={`flex gap-6 ${isMobile ? 'overflow-x-auto snap-x snap-mandatory touch-pan-x -mx-4 sm:mx-0 px-4 sm:px-0' : 'overflow-hidden'}`}>
                ...
                >
                  <div className="flex flex-col items-center gap-3 w-full">
                    {/* Video thumbnail. Was w-[504px] h-72 unconditional;
                        ...
                        sm. */}
                    <div className="w-full aspect-video sm:aspect-auto sm:h-72 bg-zinc-300 dark:bg-zinc-800 rounded overflow-hidden">
                      {video.thumbnailUrl ? (
                        ...
                        <div className="w-full h-full bg-zinc-300 dark:bg-zinc-800 transition-transform duration-300 ease-in-out group-hover:scale-110" />
                      )}
                    </div>
              ...
              {(() => {
                ...
                return relatedTopics.map((relatedTerm, index) => (
                  ...
                  >
                    <div className="flex flex-col items-center gap-3 w-full">
                      <div className="w-full aspect-video sm:aspect-auto sm:h-72 border-2 border-black dark:border-zinc-600 rounded overflow-hidden flex items-center justify-center">
                        <div className="text-center p-4 sm:p-8">
                          <div className="text-black dark:text-zinc-100 text-2xl sm:text-3xl md:text-4xl font-bold capitalize" style={{fontFamily: 'Source Serif Pro, serif'}}>
                            {relatedTerm.topic}
                          </div>
                        </div>
                      </div>
              ...
              {backgroundLoading && (
                <div className="flex-shrink-0 w-[80vw] max-w-[504px] sm:w-[504px] flex flex-col items-center justify-center gap-3" role="status" aria-label="Loading more clips">
                  <div className="w-full aspect-video sm:aspect-auto sm:h-72 bg-zinc-200 dark:bg-zinc-800 rounded overflow-hidden flex items-center justify-center">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-zinc-400 dark:border-zinc-500" aria-hidden="true" />
                  </div>
                  <div className="text-zinc-500 dark:text-zinc-400 text-base font-light font-mono">
                    Loading more clips...
                  </div>
                </div>
              )}

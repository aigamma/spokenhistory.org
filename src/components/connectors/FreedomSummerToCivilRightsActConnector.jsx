import { useState, useEffect } from 'react';

/**
 * FreedomSummerToCivilRightsActConnector - Custom connector from Freedom Summer date to Civil Rights Act date
 */
export default function FreedomSummerToCivilRightsActConnector({ fromRef, toRef }) {
  const [connectorPath, setConnectorPath] = useState({ 
    segments: [], 
    show: false 
  });

  useEffect(() => {
    const updateConnector = () => {
      if (typeof window !== 'undefined' && window.innerWidth < 1024) {
        setConnectorPath({ segments: [], show: false });
        return;
      }
      if (fromRef.current && toRef.current) {
        const fromRect = fromRef.current.getBoundingClientRect();
        const toRect = toRef.current.getBoundingClientRect();
        const scrollTop = window.pageYOffset || document.documentElement.scrollTop;

        // Start from right of Freedom Summer date badge
        const startX = fromRect.right;
        const startY = fromRect.top + fromRect.height / 2 + scrollTop - 148;

        // End at top of Civil Rights Act date badge
        const endX = toRect.left + toRect.width / 2;
        const endY = toRect.top + scrollTop - 147;

        // Calculate elbow points for right -> down -> left -> down path
        const horizontalDistance1 = 650; // How far right to go initially
        const verticalDistance1 = 900; // First vertical drop (elbow down)
        const horizontalDistance2 = Math.abs(endX - (startX + horizontalDistance1)); // Distance to go left to target
        const verticalDistance2 = endY - (startY + verticalDistance1); // Final vertical drop to Civil Rights Act date
        
        const firstElbowX = startX + horizontalDistance1;
        const firstElbowY = startY + verticalDistance1;
        const secondElbowX = endX;
        const secondElbowY = firstElbowY;
        
        const segments = [
          // Horizontal segment right from Freedom Summer date
          {
            type: 'horizontal',
            x: startX,
            y: startY,
            width: horizontalDistance1
          },
          // First vertical segment down (elbow down)
          {
            type: 'vertical',
            x: firstElbowX,
            y: startY,
            height: verticalDistance1
          },
          // Horizontal segment left to align with Civil Rights Act date
          {
            type: 'horizontal',
            x: Math.min(firstElbowX, endX), // Start from the leftmost point
            y: firstElbowY,
            width: horizontalDistance2
          },
          // Final vertical segment down to Civil Rights Act date
          {
            type: 'vertical',
            x: endX,
            y: firstElbowY,
            height: verticalDistance2
          }
        ];

        setConnectorPath({ segments, show: true });
      }
    };

    updateConnector();
    window.addEventListener('resize', updateConnector);
    window.addEventListener('scroll', updateConnector);

    return () => {
      window.removeEventListener('resize', updateConnector);
      window.removeEventListener('scroll', updateConnector);
    };
  }, [fromRef, toRef]);

  if (!connectorPath.show) return null;

  return (
    <>
      {connectorPath.segments.map((segment, index) => (
        <div
          key={index}
          className={`absolute bg-red-500 opacity-100 pointer-events-none ${
            segment.type === 'horizontal' ? 'h-px' : 'w-px'
          }`}
          style={{
            left: segment.type === 'horizontal' ? segment.x : segment.x,
            top: segment.y,
            width: segment.type === 'horizontal' ? segment.width : undefined,
            height: segment.type === 'vertical' ? segment.height : undefined,
            position: 'absolute',
            zIndex: 0
          }}
        />
      ))}
    </>
  );
}

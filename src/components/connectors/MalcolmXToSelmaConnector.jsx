...
/**
 * MalcolmXToSelmaConnector - Custom connector from Malcolm X date to Malcolm X GIF
 * Path: right → down → left → down
 */
export default function MalcolmXToSelmaConnector({ fromRef, toRef }) {
  ...
  useEffect(() => {
    const updateConnector = () => {
      ...
      if (fromRef.current && toRef.current) {
        ...
        const verticalDistance1 = 980; // First vertical drop
        const horizontalDistance2 = (startX + horizontalDistance1) - endX; // Distance to go left to target
        const verticalDistance2 = endY - (startY + verticalDistance1) + 50; // Final vertical drop to Selma date
        
        const firstElbowX = startX + horizontalDistance1;
        const firstElbowY = startY + verticalDistance1;
        
        const segments = [
          // 1. Horizontal segment right from Malcolm X date
          {
            type: 'horizontal',
            x: startX,
            y: startY,
          ...
          // 3. Horizontal segment left to align with Selma date
          {
            type: 'horizontal',
            x: endX,
            y: firstElbowY,
          ...
          // 4. Final vertical segment down to Selma date
          {
            type: 'vertical',
            x: endX,
            y: firstElbowY,

...
- **Eric Allione** ([@aigamma](https://github.com/aigamma)) — Smithsonian-grade quality overhaul. Hardened the metadata-pipeline tuning loop with a Claude Opus 4.7 external second-opinion scorer (`processor/claude_scorer.py`), the dual-scorer publication gate (`tune_with_dual_scoring`), the per-claim citation auditor (`processor/citation_check.py`), and the human-review queue (`processor/review_queue.py` producer + `src/pages/ReviewQueue.jsx` admin UI + `src/services/reviewQueue.js`); expanded `civil_rights_facts.json` ground-truth coverage from 17 to 46 entries (37 with alias lists) and replaced the fragile substring matcher in `shared.py::get_relevant_facts` with a word-boundary regex matcher that consults the aliases. Site-wide mobile-friendliness pass: built the dismissible advisory banner replacing the prior hard-block (`src/components/MobileAdvisory.jsx`), brought tap targets up to the WCAG 2.2 AA 44x44 minimum across Header, PlaylistBuilder, TopicGlossary, MobileAdvisory, VectorSearchOverlay, and FeedbackModal close affordances, added dialog semantics and `aria-modal` to all four modal overlays, and added screen-reader announcements to the two route-level loading spinners. Cross-project infrastructure: standalone Model Context Protocol server exposing the archive to Claude Desktop and Claude.ai Custom Connectors (`mcp-server/`), Firestore migration script for the llm-hyper-audio → civil-rights-history-project transition (`scripts/firestore-migrate.mjs`), Firestore security rules (`firestore.rules`), CI workflow (`.github/workflows/ci.yml`), Firebase config moved to `VITE_FIREBASE_*` env-var override pattern. Defensive hardening: input validation on all three Cloud Functions + all three MCP tools + Firebase Auth surface, type-coercion on Claude scorer + citation auditor model outputs, named firebase-admin app in `review_queue.py` to avoid default-app collision. Documentation: this contributors ledger.

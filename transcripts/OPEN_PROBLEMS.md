...
## Problem 8 — Subject-paragraph publication-blocking corrections — **RESOLVED 2026-05-24 (Session 7 Phase 1 apply-back)**

### Resolved 2026-05-24 (Session 7 Phase 1 apply-back)

Codex applied the Pass 7 Subject paragraph corrections to `transcripts/CLEANED_TRANSCRIPTS_REVIEW.md` using `transcripts/apply_subject_corrections.py`, drawing authoritative rewrites from the one-entry PRR stage files rather than from the master MD. Final apply-back coverage:

- 109 target entries processed: 106 from `subject_paragraph_corrections_pass7.json` plus three supplemental hard-stop entries called out by the Codex handoff (#96 Connor, #108 Carter, #130 Saunders).
- 109 corrected paragraphs extracted; 108 Subject lines changed from committed HEAD baseline; one stale supplemental metadata line removed.
- Current idempotency verified: `python transcripts\apply_subject_corrections.py --dry-run` reports 109 already current, 0 changes.
- `python scripts\apply_corrections.py` was run after the bulk apply-back; corrected transcript surfaces were then patched for late Pass 7 ASR-bleed catches in John Carlos, Clarence B. Jones, and Norma Mtume.
- Hard-stop Subject blocker verification passed 13/13, including Greyhound/Trailways (#9), Briggs spectator/co-counsel (#87), Carter Linden NJ (#108), Parker/Sammy Davis direction (#125), Lucy April 1968 settlement (#128), and Saunders Tennessee/1967/foot claims (#130).

Problem 8 is now closed. Downstream work remains in Problems 1, 5, 6, and 9 plus the deployment/pipeline priorities, but no further Subject paragraph apply-back is pending.

### Resolved 2026-05-24 (Pass 7 PRR diagnosis)

Pass 7 Publication Readiness Review (Session 6) audited the Subject paragraph for every audit-able entry. Results captured at `transcripts/subject_paragraph_corrections_pass7.json` (auto-generated by `transcripts/merge_pass7.py` from the per-entry PRR staging files at `transcripts/pass7_stage/entry_NNN_*.md`):

- **106 of 127 audit-able entries (83%)** have at least one Subject paragraph claim that needs a fix (`partial`, `unsupported`, or `contradicted`).
- **330 total claim-level corrections identified** — far exceeds the Session 4 Pass 4 estimate of "~120+ across ~50+ entries" because Pass 7 examined every Subject paragraph claim systematically, not just the ones flagged in passing.
- **Of the 330 corrections, 38 are `contradicted` (hard publication blockers)** — the Subject paragraph asserts something the transcript directly refutes. Examples: entry 9 (Booker on Greyhound — actually Trailways); entry 87 (Perry as Briggs v Elliott co-counsel — actually attended as spectator); entry 125 (Sammy Davis Jr. pallbearer at Mamie Till funeral — Davis died before Mamie); entry 49 (Gloria Richardson "only woman invited" at March on Washington — Josephine Baker + Lena Horne also on platform); entry 56 (Greenberg tenure "1984-2017" — died 2016).
- **Each entry's PRR stage file contains the agent's recommended corrected Subject paragraph** ready for Codex to apply.

**Historical note from the pre-apply Pass 7 handoff:**

The following text is retained for audit history; it described the state before Session 7 Phase 1 applied the corrections.

Pass 7 produced the *diagnosis* + the *corrected text* for each flagged Subject paragraph. The actual write-back to master MD (and downstream propagation to any Firestore subject-paragraph field) is the next step. Codex's task: consume `subject_paragraph_corrections_pass7.json`, locate each entry's Subject paragraph line in the master MD, replace with the corrected version, recompute downstream Firestore metadata. This is mechanical given the JSON; no further audit work needed.

**Original Codex handoff checklist (completed in Session 7 Phase 1):**
1. Iterate `subject_paragraph_corrections_pass7.json["entries"]`. For each entry, if `corrected_subject_paragraph` is non-null, replace the existing Subject paragraph in master MD (look for the `**Subject:**` line in the entry's section).
2. For entries with `claims_needing_fix` but no `corrected_subject_paragraph` extracted (parser missed it), read the entry's Pass 7 PRR staging file at `transcripts/pass7_stage/entry_NNN_*.md` and apply manually.
3. Re-run `scripts/apply_corrections.py` to propagate Subject paragraph changes to the `corrected/` output if any pipeline downstream consumes them.

### Original 2026-05-22 Session 4 punch-list (now superseded by Pass 7 resolution)

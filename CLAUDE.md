...
- **Homepage (`Home.jsx`)** leads with Dustin's tagline, then a large random rotating Featured Quote (`HeroQuote`, reads `summaries/quotes.json`), then ONE "Explore the Collection" button to `/topic-glossary`. The "145 Interviews" and "View Timeline" entry points were removed; the timeline stays directly below.

...
- **`/people` is now historic figures only, renamed "Historical Figures Referenced in Interviews."** `src/pages/PeopleCatalog.jsx` was rewritten: it drops the interviewees (now browsed under Interviews) and shows ALL 37 external figures as rich thumbnail cards, each linking to its strong `/person/:slug` reference page. This replaces the retired `FamousNames` panel, which covered only 15 of the 37 and led to "N voices" inline passages rather than the reference pages. (`FamousNames.jsx` still exists and is exported from `rag/index.js` but is no longer imported anywhere; safe to delete in a later cleanup.) Route stays `/people` for link stability; the page H1, document title, and nav label are all "Historical Figures Referenced in Interviews."
- **The themes-and-playlists book (`/topic-glossary`) nav label was renamed "Table of Contents" -> "Topics"** (page content unchanged). NOTE: Eric named the target as both "Topics" and "Contents"; "Topics" was chosen because it matches the page's content and `/topic-glossary` route and reads clearly next to "Interviews". One-word change if "Contents" is preferred.
- **Final live `MENU_ROUTES` order:** Timeline (`/`), Interviews (`/table-of-contents`), Topics (`/topic-glossary`), Historical Figures Referenced in Interviews (`/people`), Explore Interview Data (`/rag-explore`), K-12 Curriculum (`/curriculum`).

...
Project context: an open-source AI system that transforms the Library of Congress Civil Rights History Project oral history archive (roughly 250 hours of interviews across ~145 published oral histories, produced in collaboration with the Smithsonian NMAAHC) into structured, searchable metadata. (An earlier "600+ hours" figure circulated in project materials; the published collection's own time-anchored data measures ~250 hours, corrected project-wide 2026-06-10.) The Smithsonian has been scrutinizing the team's AI-generated summaries for hallucinations -- the quality bar is "Smithsonian-grade publication," not "good enough for a research demo."
...
Data-build scripts (run from repo root after a corpus change): `scripts/build_toc.py`, `scripts/build_playlist_index.py`, `scripts/build_people_index.mjs`, `scripts/build_event_network.py`, `scripts/build_loc_video_index.mjs`; plus the Pinecone-derived precompute in `rag/precompute.mjs`, `rag/precompute_concept_axes.mjs`, `rag/precompute_influence.py`, `rag/precompute_panels.mjs`, and `rag/summarize.mjs`. (`onboard_interview.py` rebuilds `toc.json`, `playlist_index.json`, and `loc_video_index.json` automatically in its stage 13.)
...
- **`mcp-server/`** -- Node MCP server exposing the archive via eight tools: five primitives (`search_transcripts`, `get_transcript`, `list_leaders`, `list_people`, `list_essays`) plus three research-pattern tools (`compare_perspectives`, `trace_evolution`, `source_for_claim`). The research patterns are also registered as MCP prompts for prompt-routing clients, and the server exposes MCP resources (`civilrights://corpus/overview`, `/leaders`, `/people`, `/essays`, `/transcript/{n}`). Pure logic is split into `lib.mjs` (unit-tested in `test/`); IO + wiring in `server.mjs`. DEPLOYED and live on Fly.io at `https://mcp.spokenhistory.org/mcp` (+ `https://civil-rights-history-mcp.fly.dev/mcp`); push-to-deploy via `.github/workflows/deploy-mcp.yml`. The public, non-gated onboarding page is `/connect` (`src/pages/Connect.jsx`); the canonical from-scratch guide is `mcp-server/GETTING_STARTED.md`.
- **`scripts/`** -- Admin and data-build scripts. The corpus-derived static artifacts are (re)built here: `build_toc.py` (toc.json), `build_playlist_index.py` (playlist_index.json), `build_people_index.mjs` (people/index.json), `build_event_network.py` (event_network.json), `build_loc_video_index.mjs` (loc_video_index.json, the compact per-entry video metadata the player reads to paint a clip's poster without fetching the whole per-entry JSON), the re-chapterization tooling (`merge_rechapter.py`, `expand_chapters.py`, `add_parts.py`, `build_rechapter_map.py`), the person-page gates (`verify_person_snippets.py`, `audit_axis_labels.py`, `classify_people_pages.py`), `strip_em_dashes.mjs`, and `diag_corpus_state.py` (the corpus-state diagnostic). Legacy Firestore/Firebase migration and media scripts also live here. Do not delete without explicit confirmation; some are reference material.

### Civil Rights MCP usage in Codex Desktop

The canonical MCP surface is eight tools and three prompts (plus MCP resources). Run `npm run smoke` from `mcp-server/` to verify the stdio server advertises all of them before debugging a client.

Primitive tools:
- `search_transcripts` - citation-grade semantic search across the archive. Use `dedupe_by_entry: true` when manually handling compare-perspectives-style requests; `include_persons` / `include_essays` widen to person pages / essay text.
- `get_transcript` - full ordered transcript for one `entry_number`.
- `list_leaders` - interviewee roster with entry numbers, LoC URLs, provenance, and audit tiers.
- `list_people` - full people catalog (165 interviewees + 37 external figures); filter via `person_type`.
- `list_essays` - curated public-domain essays catalog (23 essays, 10 topics); filter via `topic`.
...
`onboard_interview.py` is a master, idempotent, 16-stage pipeline (every stage checks for its own output and skips work already done, so it is safe to re-run): locate, bootstrap raw to corrected, resolve LoC item, LoC heal, assign entry_number, resolve LoC video, extract cue blocks, expand chapters, attach summary, assemble `entry_<N>.json`, ingest vectors to Pinecone, scaffold the person page, rebuild `toc.json` and `playlist_index.json` (plus `loc_video_index.json`), rebuild the derived cross-link network, append an AUDIT_TRAIL note, print status. It reuses `ingest_new_transcript.py` (the bootstrap and LoC-heal sub-step) rather than duplicating it.
...
| File | Purpose |
|---|---|
| `docs/TECHNICAL_PAPER.md` | **The systems-level technical paper** (2026-06-10, ~10,800 words): how the whole architecture works end to end (time substrate, audit cascade, LoC healing, onboarding, publication gate, semantic substrate, derived structures, exploration surfaces, MCP) and what it enables versus legacy archive technology. Ten data-driven figures live in `docs/figures/technical-paper/` (regenerate with `generate_figures.py` there); the published DOCX edition is `docs/TECHNICAL_PAPER.docx` (MLA-formatted: Calibri, running head with page numbers, Works Cited; rebuild with `python docs/build_technical_paper_docx.py`). Authors: Dustin O'Hara, PhD (PI & Project Director), Jack Sovelove (Co-PI & Software Developer), Eric Allione (AI Engineering Lead), Sophia Zhuk (Software Developer); UCLA, under the supervision of the Library of Congress. Every quantitative claim was independently re-derived with as-of dates (its Appendix A). NOTE: the paper uses MEASURED corpus figures (roughly 250 hours of video, ~2.0M transcribed words across the 140 holdings), not the older "600+ hours / ~5M words" framing, which its verification pass could not reconcile against the corpus's own time-anchored data. |
| `docs/CONFERENCE_PAPER.md` | **The conference edition of the technical paper** ("Improving the Experience of Observing Archived Data", 2026-06-10, ~8,300 prose words): the same 15-section architecture, authors, and verified figures as `TECHNICAL_PAPER.md`, refined for a one-sitting read for the London conference (June 2026); prose condensed ~15%, swarm-draft repetition edited out, all ten figures properly captioned. DOCX edition `docs/CONFERENCE_PAPER.docx` rebuilds with `python docs/build_conference_paper_docx.py` (imports and reuses the technical edition's MLA pipeline, plus `+implicit_figures` so captions render). The figures are shared from `docs/figures/technical-paper/`. Keep the two editions' quantitative claims in sync: both draw on the same Appendix A inventory. |
...
**Status (updated 2026-06-10):** Catalog at 202 pages (165 interviewees + 37 external figures), 100% bio coverage, verbatim interview-snippet coverage on all non-stub pages (1,021 quotes on 177 pages, gated by `scripts/verify_person_snippets.py`, 1021/1021 passing on a 2026-06-10 live run), and full portrait/environment hero imagery (0 bare pages). CAUTION: `scripts/audit_axis_labels.py` currently FAILS (exit 1: 10 mismatches on 8 pages, all near-zero positions with |position_normalized| <= 0.21), consistent with a later `ideological_spectrums.json` regeneration drifting near-center entries across the sign boundary after the `ai_reading` prose was written; the flagged pages await re-wording or re-pinning. The exclusion filter is implemented at both layers (`src/services/ragClient.js::retrieve` and `mcp-server/server.mjs::searchTranscripts`) as of commit 6f446a7. The ingestion command path is implemented in `rag/ingest.mjs` as of commit 7cbf643:
...
|---|---|---|
| `transcripts/pass2_stage/` | 87 per-entry files + INDEX.md | Session 2 Phase A Pass 2 per-entry corrections (entries #43–#132) |
...
| `transcripts/per_entry_slices/` | 138 slices + INDEX.md + manifest.json (grows with the corpus) | Session 4 cross-contamination firewall slices, each agent reads ONLY its own slice, never the master MD. Built by `transcripts/slice_master_md.py`. |
...
- In-page anchor links must NOT be written as a raw `<a href="#section">`. The app runs under `<HashRouter>` (`src/main.jsx`), so the URL hash IS the router's route; a bare fragment href overwrites the route, the router matches nothing, and the catch-all renders the 404 page. This is exactly what was 404ing the Topics page "Contents" jump-links, the accessibility skip-link, the Essays "hear the oral histories" link, and the `[N]` citation footnotes on the interview and person pages (all fixed 2026-06-13). For any in-page jump, intercept the click (`e.preventDefault()`) and call `scrollToId(id)` from `src/utils/hashScroll.js`, or use a `<button>` (the `AnchorMark` in `InterviewDetail.jsx` is the older hand-rolled version of the same pattern). A route link written by hand must use the `#/path` form (note the leading slash, as in `ContentDirectory.jsx`), but prefer `<Link>`.
...
- A "See this in context" snippet clip taking ~1-2s to start on a long interview is NOT downloading the whole interview, and is NOT a bug to chase. Empirically (2026-06-10 probe of the live `tile.loc.gov` URLs): each LoC recording is one faststart MP4 (1.5-2.4 GB) whose **moov atom (the seek index) sits at the front and grows with duration**, ~40 KB for a 73-second clip but **5.4 MB for a 3-hour interview**. Any seek (and even play-from-0) must download that entire atom before the first frame, then the clip's own byte range (~1 MB) loads via a range request. So a deep clip fetches ~6.5 MB, the moov plus the clip, never the multi-GB file; `src/utils/mediaClip.js::seekThenPlay` enforces the seek-before-play that keeps it byte-bounded. The interview page only *feels* faster because commit `a8976e3` made its hero `preload="none"` (instant poster, zero download until you press play); the snippet auto-plays on open, so it pays the moov immediately. The moov is the hard floor for MP4 seeking and there is no client-side way to shrink it. **HLS (`video_stream_url`) was evaluated as an escape and rejected:** LoC's IIIF stream transcodes segments on demand, and a *cold* arbitrary-offset segment measured 2.8s+ to first byte (sometimes a >20s stall), so HLS would make the cold long tail slower, not faster; warm (CDN-cached) segments are fast but not something we can guarantee. The shipped mitigations are perceptual + structural, not a moov shrink: the hero's deferred preload, the curriculum's `lazy` poster-gate, and the shared `loc_video_index.json` (one ~8 KB-gzipped fetch per session so every snippet's poster paints with no per-entry JSON fetch). `tile.loc.gov` serves `cache-control: max-age=1yr`, so the moov is cached after the first clip into an interview; repeat clips into the same interview are already fast.
...
- **Tier-vocabulary doc cleanup** (`3f4249f`, `ed18acc`, `07438c2`). At the time, the vocabulary value `high` was a placeholder that never appeared in the corpus, so these commits updated mcp-server/USAGE_GUIDE.md, the `source_for_claim` MCP prompt, rag/INTERACTIVE_FEATURES_DESIGN.md, and rag/README.md to list the then-real 5 tier values (low / medium / publication-block / not-auditable / ingestion-only). (UPDATE 2026-06-10: OBSOLETE. After the Pass 10 rescore, `high` is the dominant tier; the measured distribution in pipeline_output is high 133 / not-auditable 3 / ingestion-only 3 / publication-block 1 over 140 entries, and the docs were re-corrected accordingly.)
...
- **The default is animations ON unless the OS asks for reduced motion**, which was already the behavior (no code change needed) and is now the documented intent: `getInitialPaused()` in `useAnimationPreference.js` and the no-FOUC script in `index.html` both default to not-paused unless `prefers-reduced-motion: reduce` is detected. Eric's rationale: a default-on state makes the button read "Pause Animations" (a clear, discoverable affordance) rather than leading with the resume-state "Allow Animations" label; devices with a detectable reduce-motion setting remain the deliberate exception and still default to paused. (Note: a machine with reduced-motion set, e.g. a motion-sensitive developer's, will correctly see the page default to paused and the button read "Allow Animations"; that is the exception working as specified, not a regression. The resume-state label was renamed "Play Animations" -> "Allow Animations" on 2026-06-04, Eric.)

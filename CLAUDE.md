...
Three small header/nav changes landed on `master`. All three are in `src/components/common/Header.jsx`; no new files. Durable facts:
...
- **Menu reorder: "People & Interviews" now sits above "Explore Interview Data"** in `MENU_ROUTES`. New order: Timeline, Table of Contents, People & Interviews, Explore Interview Data, K-12 Curriculum. The human-facing browse (interviewees and the figures they discuss) precedes the analytical maps-and-retrieval surface.

Verification: same stack as the 2026-06-02 batch (local `vite build` segfault stands). Gated on the esbuild per-file parse check of `Header.jsx` plus the whole-app import-graph check (clean, ~255ms) and a zero-em-dash check of the edited file. The Netlify Linux build remains the deploy gate.

...
The actual Pinecone push requires Eric's Voyage + Pinecone credentials in `rag/.env.local`. Estimated cost: ~$0.05 in Voyage embedding for the 191-vector initial push; re-runs are idempotent on content hash and only re-embed changed pages.

...
Three linked navigation features (branch `feat/share-and-global-search`; preview-only until merged to `master`). The thread is that a video segment on this site is identified by `(entryNumber, startSeconds, endSeconds)`, which is a URL, so it is shareable, deep-linkable, and the natural funnel target for search.
...
**Global search (`src/components/GlobalSearch.jsx`, mounted in `Header.jsx`).** An always-visible, low-prominence search (a hairline + muted magnifier) across the top of every framed page. Debounced live query through `ragClient.retrieve` (the existing `/retrieve` Netlify function, Pinecone + Voyage) with an `AbortController`. Results funnel three ways: a **Playlist** CTA (`/playlist-builder?keywords=...`, also the Enter action, the primary funnel), distinct **Interviews**, and time-anchored **Clips** that open `/interview/N?t=&end=` (reusing the segment deep-link, so selecting a clip lands the reader listening). Relevance is shown as **evidence** (the matched passage + a similarity score), not model-generated prose, which is the safer choice on a Smithsonian-grade archive; a generated one-liner could be layered on later. Dismisses on Escape / outside-click / route change; degrades gracefully when `/retrieve` is unavailable (the client-side playlist funnel still works, so it is functional even on `localhost` where the Netlify function is absent).

**Known scope note:** the header (and thus the global search + page-share) renders only on `Layout`-wrapped routes. The player routes (`/interview/:entryNumber`, `/interview-player`, `/clip-player`) render without `Layout`, so they have no header search; the interview page carries its own richer share controls instead. The header is not sticky-on-scroll (matches the prior header); pinning it is a one-line follow-up if wanted.
